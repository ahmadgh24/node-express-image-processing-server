...
describe('module 4', () => {
  context('imageProcessor.js', () => {
    it('You should export the image processor function @create-the-image-processor', () => {
      expect(
        ...
        'Did you remember to export the `imageProcessor()`?'
      ).to.equal('function');
    });
    ...
    it('the image processor function should return a promise @instantiate-a-promise', async () => {
      try {
        ...
        throw new Error(
          'Did you return a promise from the `imageProcessor()`?'
        );
      }
    });
    ...
    it('should define a function uploadPathResolver that takes a filename as its argument @create-upload-path-resolver', () => {
      let uploadPathResolver;
      try {
        uploadPathResolver = rewire('../../api/src/imageProcessor').__get__(
          'uploadPathResolver'
        );
      } catch (err) {
        throw new Error(
          'Did you remember to define the `uploadPathResolver()`?'
        );
      }
    ...
    it('should assign an instantiation of the Worker class to a constant representing the resizeWorker @instantiate-the-resize-worker', async () => {
      ...
      imageProcessor.__set__(
        'pathToResizeWorker',
        path.resolve('./test/module-4/resizeStub.js')
      );
      imageProcessor.__set__(
      ...
      expected = [
        path.resolve(__dirname, '../../api/src/resizeWorker.js'),
        {
          workerData: {
            source: path.resolve(
              __dirname,
              '../../api/uploads/hereIsTheFile.png'
            ),
            destination: path.resolve(
              __dirname,
              '../../api/uploads/resized-hereIsTheFile.png'
            ),
          },
        },
      ];
      ...
      expected = [
        path.resolve(__dirname, '../../api/src/monochromeWorker.js'),
        {
          workerData: {
            ...
            destination: path.resolve(
              __dirname,
              '../../api/uploads/monochrome-hereIsTheFile.png'
            ),
          },
        },
      ];
    ...
    it("should resolve the promise on resizeWorker's 'message' event @resize-on-message", async () => {
      ...
      imageProcessor.__set__(
        'pathToResizeWorker',
        path.resolve('./test/module-4/resizeWorkerProxy.js')
      );
      imageProcessor.__set__(
        'pathToMonochromeWorker',
        path.resolve('./test/module-4/monochromeFast.js')
      );
    ...
    it("should reject the promise on resizeWorker's 'error' event @resize-error-event", async () => {
      ...
      imageProcessor.__set__(
        'pathToResizeWorker',
        path.resolve('./test/module-4/resizeWorkerProxyError.js')
      );
      imageProcessor.__set__(
        'pathToMonochromeWorker',
        path.resolve('./test/module-4/monochromeStub.js')
      );
    ...
    it("should reject the promise on resizeWorker's 'exit' event when process exit's  > 0 @resize-exit-event", async () => {
      ...
      imageProcessor.__set__(
        'pathToResizeWorker',
        path.resolve('./test/module-4/resizeWorkerProxyExitCode1.js')
      );
      imageProcessor.__set__(
        'pathToMonochromeWorker',
        path.resolve('./test/module-4/monochromeStub.js')
  ...
  context('router.js', async () => {
    ...
    it('should add a call to imageProcessor passing in the image in our post request @invoke-the-image-processor', async () => {
      ...
      await request(app)
        ...
        .attach(
          'photo',
          path.resolve(__dirname, '../../client/photos/ullr.png')
        )
        .expect(201);
      ...
      expect(
        files.includes('ullr.png'),
        "Did you call `imageProcessor()` inside the `'/upload'` route?"
      ).to.be.true;
      expect(
        files.includes('resized-ullr.png'),
        "Did you call `imageProcessor()` inside the `'/upload'` route?"
      ).to.be.true;
      expect(
        files.includes('monochrome-ullr.png'),
        "Did you call `imageProcessor()` inside the `'/upload'` route?"
      ).to.be.true;
    });
  });
  ...
  context('photo-viewer.html', () => {
    ...
    beforeEach(done => {
      JSDOM.fromFile(
        path.resolve(__dirname, '../../client', 'photo-viewer.html')
      ).then(dom => {
    ...
    it('should add an img tag with an src of monochrome-ullr.png @add-monochrome-image', () => {
      expect(
        ...
        R.pathOr(undefined, [1, 'attributes', 'src', 'value'], images)
      );
    });
    ...
    it('should add an img tag with an src of resized-ullr.png @add-resized-image', () => {
      expect(
        ...
        R.pathOr(undefined, [2, 'attributes', 'src', 'value'], images)
      );
    });
  });
});
